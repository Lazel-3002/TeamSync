...
1. `evalJS()` - 41 edges
2. `bindUI()` - 40 edges
3. `handleDataMessage()` - 38 edges
4. `showToast()` - 28 edges
5. `spawnPeer()` - 28 edges
6. `waitFor()` - 26 edges
7. `cleanupPeer()` - 22 edges
8. `initUserSettings()` - 21 edges
9. `createRoom()` - 19 edges
10. `unoIsHost()` - 18 edges
...
Cohesion: 0.05
Nodes (33): appendFileMsg(), badWordsList, badWordsRegex, chatBlobUrls, closeJoinRequestNote(), fileBuffer, filterActivityCards(), FOCUS_CARD_TITLES (+25 more)
...
Cohesion: 0.18
Nodes (12): applyAudioBitrateToPeers(), applyRoomNoiseSuppression(), canManageRoom(), getAudioBitrate(), getShareableTurn(), handleFounderLeft(), loadAIFilter(), loadRoomBans() (+4 more)
...
Cohesion: 0.12
Nodes (25): applyIceEscalationPolicy(), applySharedTurn(), applySpeakerTo(), applySpeakerToAll(), attemptIceRestart(), createPeerConnection(), detectTunnelInterference(), diagnoseIceFailure() (+17 more)

### Community 10 - "E2E Test: MQTT/First Run"
Cohesion: 0.19
Nodes (14): { spawnPeer, cleanupPeer, waitFor }, { spawnPeer, cleanupPeer, waitFor, evalJS, createRoom }, clickWhenReady(), createRoom(), joinRoom(), setValueWhenReady(), spawnPeer(), waitFor() (+6 more)

### Community 11 - "E2E Test Harness"
Cohesion: 0.25
Nodes (6): waitForPeerConnected(), assert, { spawnPeer, cleanupPeer, createRoom, joinRoom, waitForPeerConnected, evalJS }, assert, inboundRtpStats(), { spawnPeer, cleanupPeer, createRoom, joinRoom, waitForPeerConnected, evalJS }
...
Cohesion: 0.21
Nodes (22): applyPttMode(), bindUI(), canModerateTarget(), clearFocusInlineLayout(), ensureFocusControlsVisible(), enterFocus(), exitFocus(), initActivitiesUI() (+14 more)
...
Cohesion: 0.13
Nodes (29): addVideoCard(), applyMicState(), attachVideo(), broadcast(), broadcastTo(), checkTextWithAI(), closeActiveControlSession(), closeCtrlModal() (+21 more)
...
Cohesion: 0.15
Nodes (31): APP_THEMES, applyMicrophoneVolume(), applySpeakerVolume(), applyUserLanguage(), applyUserTheme(), fillAudioDeviceSelect(), formatUserTime(), getUserLanguage() (+23 more)

### Community 66 - "Ad Blocker Dependency"
Cohesion: 0.24
Nodes (8): cleanupPeer(), fs, inspectFamily(), path, run(), {
  spawnPeer,
  cleanupPeer,
  createRoom,
  evalJS,
...
Cohesion: 0.18
Nodes (27): initLuckyWheel(), afterNight(), armPhaseTimer(), castVote(), clearPhaseTimer(), directResult(), hunterShot(), kill() (+19 more)

### Community 69 - "Poke Feature Init"
Cohesion: 0.15
Nodes (14): initPoke(), assert, audioState(), setFounderToggle(), {
  spawnPeer,
  cleanupPeer,
  createRoom,
  joinRoom,
